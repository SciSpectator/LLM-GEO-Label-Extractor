...
def run_pipeline(args: argparse.Namespace, stop_evt) -> None:
    ...
    # The pipeline materializes series context straight from the input
    # database and points GSE_META_CACHE at it. Without honouring that, this
    # step ignores context it already has and fetches it again from NCBI --
    # which on a compute node with no outbound network hangs rather than
    # fails, and under --no-scrape leaves Phase 1b with no context at all.
    meta_path = Path(os.environ.get("GSE_META_CACHE")
                     or (HERE / "gse_meta_scraped.json"))
    ...
    if meta_path.exists():
        try:
            ...
            gse_meta = {}
        log(f"[run] series context: {len(gse_meta):,} GSEs from {meta_path}")
