...
#: Attempts before a call is treated as a dead server rather than a blip.
#: Generous, because a busy server does stall, but finite, because an engine
#: that failed to start never recovers and holding the GPU helps no one.
_GIVE_UP_AFTER = int(os.environ.get("PHASE1_MAX_RETRIES", "60"))
    ...
    while True:
        try:
            ...
            if 400 <= e.code < 500 and e.code not in (408, 429):
                ...
                raise
            attempt += 1
            # A server that never came up answers 502 forever, and retrying it
            # forever looks like work: a cluster shard spent ninety minutes on
            # one card sending requests into a dead engine, reporting only
            # "transient retries". Past a point this is not a blip, and failing
            # loudly frees the card and shows the real error.
            if attempt >= _GIVE_UP_AFTER:
                raise RuntimeError(
                    f"{out_name}: gave up after {attempt} attempts against "
                    f"{_CHAT_URL}; last error {e!r}. The model server is not "
                    f"answering — check its log rather than this one."
                ) from e
            if attempt % 20 == 0:
                print(
            ...
            if attempt >= _GIVE_UP_AFTER:
                raise RuntimeError(
                    f"{out_name}: gave up after {attempt} attempts against "
                    f"{_CHAT_URL}; last error {e!r}. The model server is not "
                    f"reachable — check its log rather than this one."
                ) from e
            if attempt % 20 == 0:
                print(
