...
def _choice_index(verdict) -> int:
    """The candidate index a verdict names, or -1 when it names none.

    The model is asked for a single integer and usually returns one, but it can
    answer with a list, a string, or a float. A run that reached the end of
    extraction died here on `'<=' not supported between instances of 'int' and
    'list'` after nearly three hours of work, so a malformed answer is read as
    "no choice" and the caller rejects it, exactly as it rejects an out-of-range
    integer.
    """
    value = verdict.get("choice", -1)
    if isinstance(value, bool):
        return -1
    if isinstance(value, (list, tuple)):
        value = value[0] if len(value) == 1 else -1
    try:
        return int(value)
