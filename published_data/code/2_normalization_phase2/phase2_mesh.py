...
class Phase2Mesh:
    ...
    def _expand_shortform(self, label: str, col: str, context: str) -> str | None:
        """Expand a bare acronym to its full biomedical term using GSE context,
        or None to keep it verbatim. The expansion then re-enters the cascade."""
        ctx_block = _format_context_block(context)
        user = (
            f"{ctx_block}"
            f"Field: {col}\n"
            f"Abbreviation: {label}\n\n"
            f"Expand it to the full biomedical term for THIS study, or "
            f"reply KEEP.\nTERM:"
        )
        text = self._call_lm(_SHORTFORM_SYSTEM, user)
        ms = re.findall(
            r"TERM\s*:\s*(\S.*?)\s*$", text, flags=re.IGNORECASE | re.MULTILINE
        )
        if not ms:
            return None
