Table S9. Baseline comparison on the same data. coverage_of_extracted_labels, accuracy_answerable_slice and f1_answerable_slice are deterministic and reproducible from the shipped Phase 2 outputs + reference DBs. accuracy_manual_gold (rule-based extraction) is scored against the SAME author-held manual gold as Table 1/S1 (ALE curation for Sex/Age; CREEDS/metadata for T/C/T); that gold now ships in ../S01_manual_benchmark/ together with its scorers.

Reproduce the two coverage rows:
  python3 s19_baseline_coverage.py ../../published_data/data/06_final_labels_repaired/LLM_labels_all_samples_FINAL.csv.gz ../../published_data/reference/mesh.sqlite out.csv
...
Metric rows in baseline_comparison.csv:
  ...
  coverage_of_extracted_labels
                              ...
                              369,311) that carry a MeSH or Cellosaurus identifier. the corpus-composition table reports
  ...
  accuracy_answerable_slice   accuracy on the database-answerable slice; the "ours" column equals the
                              instance-weighted accuracy in ../S03_database_resolution/db_resolution.csv
  ...
  f1_answerable_slice         F1 on the database-answerable slice; the "ours" column equals the
                              instance-weighted F1 in ../S03_database_resolution/db_resolution.csv
...
they are scored against ships in ../S01_manual_benchmark/ (consistent with Table 1/S1).

The Condition denominator is 647,007, the same count Table 1, Table S2, the consolidated extraction-yield table and
the label-specific evaluation table use. An earlier version of the script tested only for the exact string
