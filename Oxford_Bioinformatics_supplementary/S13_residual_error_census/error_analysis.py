...
def main() -> int:
    ...
    ground = {r["field"]: r for r in csv.DictReader(
        open(os.path.join(eval_dir, "supporting/metadata_grounding", "grounding_audit.csv")))}
    ...
    with open(os.path.join(out_dir, "acronym_residue.csv"), "w", newline="") as fh:
        ...
        adj = os.path.join(eval_dir, "S13_residual_error_census", "acronym_audit_summary.csv")
...
def acronym_recoverable(eval_dir):
    ...
    """
    p = os.path.join(eval_dir, "S13_residual_error_census", "acronym_audit_summary.csv")
...
def treatment_benchmark(eval_dir):
    ...
    p = os.path.join(eval_dir, "S01_manual_benchmark", "manual_gold_Treatment.csv")
    with open(p, newline="") as fh:
...
def field_scope_rows(eval_dir):
    ...
    """
    p = os.path.join(eval_dir, "supporting/field_scope", "field_scope_audit.csv")
    with open(p, newline="") as fh:
        return [r for r in csv.DictReader(fh) if r["action"] == "removed"]


def genuine_sexcond(eval_dir):
    p = os.path.join(eval_dir, "S12_integrity_screens", "still_failing.csv")
    with open(p, newline="") as fh:
...
def unattested(eval_dir):
    ...
    p = os.path.join(eval_dir, "S07_gse_shortform_audit", "E_gse_shortform_audit.csv")
    with open(p, newline="") as fh:
