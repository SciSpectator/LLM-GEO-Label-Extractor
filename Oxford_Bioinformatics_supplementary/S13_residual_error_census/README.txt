Table S13. Residual-error census — what is still wrong in the corpus the paper
reports.
...
Result files in this folder:
  ...
  acronym_residue.csv          what share of acronym-shaped Condition labels was
                               resolved, and for the rest, why not
  acronym_audit.csv            one row per unresolved acronym-and-study pair
  acronym_audit_summary.csv    the same rolled up; error_analysis.py reads it
...
Run (from this folder):
  python3 error_analysis.py \
    ...
    ../../published_data/reference/mesh.sqlite ../../published_data/reference/cellosaurus.sqlite \
    .. ../supporting/age_analysis/age_composition.py \
...
The unresolved acronym residue is split deterministically by error_analysis.py,
...
Inputs shipped for this analysis:
  ../../published_data/data/06_final_labels_repaired/LLM_labels_all_samples_FINAL.csv.gz
  ../../published_data/reference/{mesh,cellosaurus}.sqlite
  ../supporting/age_analysis/age_composition.py   (the Age classifier, reused not restated)
  ../S12_integrity_screens/still_failing.csv
  ../S07_gse_shortform_audit/E_gse_shortform_audit.csv
