Table S18. Scoring of multi-label cells. Per-label precision on cells that carry
more than one label in a field.

This is a judged evaluation, not a deterministic one. It measures precision only:
a sample's cell can be scored for whether each label shown is supported by that
sample's own raw metadata, but no resource enumerates every label that should
have been there, so recall is not estimable and is not reported.
