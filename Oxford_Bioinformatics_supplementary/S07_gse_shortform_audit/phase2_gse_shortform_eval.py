...
PKG = os.path.abspath(os.path.join(HERE, "..", "..", "published_data"))
...
if GSEM.startswith("Directory to"):
    GSEM = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
        PKG, "data", "01_input_metadata", "gse_meta_scraped.json.gz")
if MESH.startswith("Directory to"):
    MESH = sys.argv[3] if len(sys.argv) > 3 else os.path.join(PKG, "reference", "mesh.sqlite")
if OUT.startswith("Directory to"):
    OUT = sys.argv[4] if len(sys.argv) > 4 else HERE
